...
public abstract class Algorithmen {
    ...
    /**
     * Ueberprueft, ob das eingebene Wort Element der vom Automaten akzeptierten Sprache ist.
     * 
     * @param   pAutomat    endlicher Automat
     * @param   pWort       Zeichenfolge
     *  
            ...
            for(int k = 0; k < Math.pow(pLaengeMax, pTiefe); k++) {
                ...
                while(!pE.istAmEnde()) {
                    for(int i = 0; i < Math.pow(anzahl, pLaengeMax - pTiefe - 1); i++) {
                        pEingaben.setContent(pEingaben.getContent() + pE.gibAktuelles().bezeichnung);
                        pEingaben.next();
                    }
                    pE.zumNaechsten();
                }
